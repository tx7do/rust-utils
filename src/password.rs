...
// ---------------------------------------------------------------------------
// RSA 密码策略(加密/解密,非哈希;对应 Go 版 RSACrypto)
// ---------------------------------------------------------------------------

/// RSA-OAEP-SHA256 加密/解密策略(输出标准 base64)。
///
/// 注意:这是可逆加密而非哈希,Go 版中它同样不实现 `Crypto`
/// 验证接口,而是提供成对的 Decrypt。
    ...
    bytes
        ...
        .map(|c| {
            ...
            format!(
                ...
                if c.len() > 1 { B64[(n >> 6 & 63) as usize] as char } else { '=' },
                if c.len() > 2 { B64[(n & 63) as usize] as char } else { '=' },
            )
        })
        .collect()
}
...
// ---------------------------------------------------------------------------
// ECDSA 密码策略(签名式校验;对应 Go 版 ECDSACrypto)
// ---------------------------------------------------------------------------

/// 基于 ECDSA P-256 签名的密码策略:encrypt 输出签名
/// `ecdsa$<r十进制>$<s十进制>`,verify 用公钥验签。
/// 注意:同一密码两次 encrypt 产生不同签名(含随机数),与 Go 版一致。
...
impl Crypto for ECDHCrypto {
    ...
    fn test_rsa_crypto() {
        ...
        assert!(algo.export_private_key().unwrap().contains("BEGIN RSA PRIVATE KEY"));
        assert!(algo.export_public_key().unwrap().contains("BEGIN RSA PUBLIC KEY"));
    }
